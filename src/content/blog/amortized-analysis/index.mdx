...
<Callout title="CLRS 17.1-3" variant="exercise">
  使用**聚合分析**确定每个操作的摊还代价。
  
  <Callout variant="answer" defaultOpen={true}>
...
<Callout title="CLRS 17.2-2" variant="exercise">
  使用**核算法**确定每个操作的摊还代价。

  <Callout variant="answer" defaultOpen={true}>
    Let $c_i =$ cost of $i\th$ operation.
    $$
...
<Callout title="CLRS 17.3-2" variant="exercise">
  使用**势能法**确定每个操作的摊还代价。
