...
![fastfetch](/src/assets/images/fastfetch-screenshot.webp)
...
|            [yazi](https://yazi-rs.github.io/)             |            Blazing fast terminal file manager written in Rust, based on async I/O.            |  finder  |
...
- 文件查询:
  ...
  使用 pf (preview file) 别名快速预览和选择文件。调用 fzf 进行交互式模糊查找，并使用 bat 在右侧窗口实现语法高亮，预览选中文件的前 500 行内容。可以通过 <kbd>Shift</kbd> + <kbd>Up</kbd> 和 <kbd>Shift</kbd> + <kbd>Down</kbd> 翻页，提高效率。如下图所示：

  ![](/src/assets/images/pf_screenshot.webp)
...
- 内容查询:
  ...
  使用 rfv (ripgrep-fzf-vim) 函数进行全文内容搜索并在 Vim 中打开结果。搜索结果将实时展示在 fzf 窗口中，并通过 bat 预览文件的相关内容。绑定按键 <kbd>Enter</kbd> 和 <kbd>Ctrl</kbd> + <kbd>O</kbd> 以实现在 Vim 中打开搜索结果，便于进行进一步的编辑。如下图所示：

  ![](/src/assets/images/rfv_screenshot.webp)
...
在安装 zinit 的过程中，它会自动接管 `~/.zshrc`，并向其中添加其相关配置如下：
...
![](/src/assets/images/fzf-tab-screenshot.jpg)
...
![](/src/assets/images/shell-screenshot.jpg)
...
```bash title="~/.tmux.conf" showLineNumbers
# List of plugins
...
- 安装 [mas](https://formulae.brew.sh/formula/mas)，一个用于管理 Mac App Store 上的应用的命令行工具：
  ...
  brew install mas
  ```
- 按需设置 Homebrew 相关[环境变量](https://docs.brew.sh/Manpage#environment)：
  ```bash
  export HOMEBREW_DOWNLOAD_CONCURRENCY=2 # 设置 Homebrew 下载并发数
...
```bash
alias upgrade='brew update && brew upgrade --greedy && mas upgrade && brew cleanup --prune=all'
```
...
2. `brew upgrade`：将系统中已安装的所有 Homebrew 软件包升级到最新版本。
    ...
    默认情况下，拥有 `auto_updates true` 标记（具有自动更新功能）的软件不会被自动更新，若希望强制将所有应用都通过 Homebrew Cask 进行更新，则可以使用 `brew upgrade --greedy`；

4. `mas upgrade`: 一键更新从 Mac App Store 安装的应用程序；

5. `brew cleanup --prune=all`: 用于清理 Homebrew 安装的软件包时产生的临时文件和缓存，以释放磁盘空间。

执行结果样例如下：

![](/src/assets/images/mac-upgrade.jpg)
...
- 使用 rustup 更新 Rust 工具链：
    ```bash
    rustup update
    ```

- 手动同步 [atuin](https://github.com/atuinsh/atuin) Shell 记录：
    ```bash
    atuin sync
    ```
